...
public class StudentsDB {
	void RetriveAllStudentDetails() {
		   try {
			   Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/ashutosh", "root", "root");
	...
	void RetriveStudentDetail(int id) {
		   try {
			   Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/ashutosh", "root", "root");
			   System.out.println("Connected With the database successfully");
			           PreparedStatement preparedStatement=connection.prepareStatement("select * from students where std_id=?");
	...
	void InsertStudentDetail(String std_name,int std_age,String std_city,String std_phone,String std_dept)
	{
		  try {
			  			Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/ashutosh", "root", "root");
			  			System.out.println("Connected With the database successfully");
			  			PreparedStatement preparedStatement=connection.prepareStatement("insert into students(std_name,std_age,std_city,std_phone,std_dept) values(?,?,?,?,?)");
	...
	void DeleteStudentDetail(int std_id) {
		  try {
			  		  Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/ashutosh", "root", "root");
			  		  System.out.println("Connected With the database successfully");
			          PreparedStatement preparedStatement=connection.prepareStatement("delete from students where std_id=?");
	  ...
	  void UpdateStudentDetail(int std_id,String std_name,int std_age,String std_city,String std_phone,String std_dept)
	  {
		  try {
				Connection	 connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/ashutosh", "root", "root");
				System.out.println("Connected With the database successfully");
		        PreparedStatement preparedStatement=connection.prepareStatement("update students set std_name=?,std_age=?,std_city=?,std_phone=?,std_dept=? where std_id=?");
		        preparedStatement.setString(1,std_name);
		        preparedStatement.setInt(2,std_age);
		        preparedStatement.setString(3,std_city);
